  ...
  public void run() throws IOException {
	//Use BufferedReader rather than RandomAccessFile; it's much faster
	  ...
	  PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter("billboard.out")));
	    ...
	    int[] xC = new int[6];
	    int[] yC = new int[6];
	  xC[0] = Integer.parseInt(st.nextToken());
	  yC[0] = Integer.parseInt(st.nextToken());
	  xC[1] = Integer.parseInt(st.nextToken());
	  yC[1] = Integer.parseInt(st.nextToken());
	  st = new StringTokenizer(reader.readLine());
	  xC[2] = Integer.parseInt(st.nextToken());
	  yC[2] = Integer.parseInt(st.nextToken());
	  xC[3] = Integer.parseInt(st.nextToken());
	  yC[3] = Integer.parseInt(st.nextToken());
	  st = new StringTokenizer(reader.readLine());
	  xC[4] = Integer.parseInt(st.nextToken());
	  yC[4] = Integer.parseInt(st.nextToken());
	  xC[5] = Integer.parseInt(st.nextToken());
	  yC[5] = Integer.parseInt(st.nextToken());
	  
	  int totalArea = (xC[1]-xC[0])*(yC[1]-yC[0])+(xC[3]-xC[2])*(yC[3]-yC[2]);
	  int totalAreaCovered = 0;
	  
	  //Billboard 1
	  if(xC[4] < xC[1] && xC[5] > xC[0]){ //Qualifier
		  if(yC[4]<yC[1]&&yC[5]>yC[0]){ //Qualifier
			  int leftXCoordinate = 0;
			  if(xC[4]<xC[0]){
				  leftXCoordinate=xC[0];
			  }else{
				  leftXCoordinate=xC[4];
			  }
			  int rightXCoordinate = 0;
			  if(xC[5]>xC[1]){
				  rightXCoordinate=xC[1];
			  }else{
				  rightXCoordinate=xC[5];
			  }
			  
			  int leftYCoordinate = 0;
			  if(yC[4]<yC[0]){
				  leftYCoordinate=yC[0];
			  }else{
				  leftYCoordinate=yC[4];
			  }
			  int rightYCoordinate = 0;
			  if(yC[5]>yC[1]){
				  rightYCoordinate=yC[1];
			  }else{
				  rightYCoordinate=yC[5];
			  }
			  totalAreaCovered+=(rightXCoordinate-leftXCoordinate)*(rightYCoordinate-leftYCoordinate);
		  }
	  }
	  
	  //Billboard 2
	  if(xC[4] < xC[3] && xC[5] > xC[2]){ //Qualifier
		  if(yC[4]<yC[3]&&yC[5]>yC[2]){ //Qualifier
			  int leftXCoordinate = 0;
			  if(xC[4]<xC[2]){
				  leftXCoordinate=xC[2];
			  }else{
				  leftXCoordinate=xC[4];
			  }
			  int rightXCoordinate = 0;
			  if(xC[5]>xC[3]){
				  rightXCoordinate=xC[3];
			  }else{
				  rightXCoordinate=xC[5];
			  }
			  ...
			  int leftYCoordinate = 0;
			  if(yC[4]<yC[2]){
				  leftYCoordinate=yC[2];
			  }else{
				  leftYCoordinate=yC[4];
			  }
			  int rightYCoordinate = 0;
			  if(yC[5]>yC[3]){
				  rightYCoordinate=yC[3];
			  }else{
				  rightYCoordinate=yC[5];
			  }
			  totalAreaCovered+=(rightXCoordinate-leftXCoordinate)*(rightYCoordinate-leftYCoordinate);
	  ...
	  int finalArea = totalArea - totalAreaCovered;
	  writer.println(finalArea);
	   reader.close();
	    writer.close();
  }
